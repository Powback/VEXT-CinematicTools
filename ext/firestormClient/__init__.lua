...
ve_firestorm_improve = require "firestorm_improve"
ve_firestormsunset = require "firestormsunset"
...
function CinematicToolsClient:RegisterVars()
	...
	self.m_firestorm_improve = json.decode(ve_firestorm_improve:GetPreset())
	self.m_firestormsunset = json.decode(ve_firestormsunset:GetPreset())
...
function CinematicToolsClient:OnUpdateInput(p_Delta)
	...
	if InputManager:WentKeyDown(InputDeviceKeys.IDK_F1) then
		...
		self:LoadPreset(self.m_firestorm_improve)
	end
	
	if InputManager:WentKeyDown(InputDeviceKeys.IDK_F6) then
		-- WebUI:ExecuteJS('document.location.reload()')
		self:LoadPreset(self.m_firestormsunset)
	end
